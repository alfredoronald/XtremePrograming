import { getPrograDB } from "../database/pairProgrammingDB.js";
...
export class ComunicacionController {
  ...
  static async Iniciar(idPiloto, idCopiloto) {
    try {
      ...
      const resultadoSesion = await pool
        .request()
      ...
      await pool.request().input("idSesion", sql.Int, idSesion)
        .query(`INSERT INTO ProgresoSesion (id_sesion, avance_azul, avance_verde) 
                        VALUES (@idSesion, 0, 0);`);

      console.log(
        `[CONTROL] Comunicación Iniciada de forma exitosa para la sesión #${idSesion}`,
      );
  ...
  static async ActualizacionTiempoRealAzul(idSesion, idCopiloto) {
    try {
      ...
      const resultado = await pool
        .request()
        .input("idSesion", sql.Int, idSesion).query(`UPDATE ProgresoSesion 
